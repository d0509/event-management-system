...
@section('content')
    <div class="container">
        ...
        @if (Auth::user()->role_id == config('site.roles.admin'))
            <form method="POST" action="{{ route('admin.profile.update', ['profile' => Auth::user()]) }}"
                enctype="multipart/form-data">
            @else
                <form method="POST" action="{{ route('company.profile.update', ['profile' => Auth::user()]) }}"
                    enctype="multipart/form-data">
        @endif
        @csrf
        @method('PATCH')
        ...
        <div class="form-outline mb-4">
            ...
            <select class="form-control form-select-lg" aria-label="Default select example" name="city_id" id="city_id">
